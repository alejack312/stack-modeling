...
test suite for inheritanceModel {
    ...
    assert {no c, p, a: Class | p in c.inherits and a in p.^inherits and a in c.inherits} is sat

    // 4. Inheritance is not symmetric
    assert {no c1, c2: Class | c1 in c2.inherits and c2 in c1.inherits} is sat

    // 5. Multiple roots are possible
    assert {some disj c1, c2: Class | no c1.inherits and no c2.inherits} is sat

    // 6. Interface can be unimplemented
    assert {some i: Interface | no c: Class | i in c.implements} is sat

    // 7. Class can exist without implementing any interface
    assert {some c: Class | no i: Interface | i in c.implements} is sat

    // 8. Class cannot implement the same interface as an ancestor
    assert {
        no c: Class, i: Interface | i in c.implements and 
        (some p: c.^inherits | i in p.implements)
    } is sat

    // 11. Hierarchy can be deeper than two levels
    assert {some c, p, a: Class | p in c.inherits and a in p.inherits} is sat

    // 12. Resolution policy: RequireOverride enforces explicit override
    assert { 
        no c: Class | c.policy = RequireOverride and 
    ...
    assert { 
        no c: Class | c.policy = MergeAll and 
    ...
    assert {some c: Class, p: Class | p in c.inherits} is sat
...
test suite for singleInheritance {
    ...
    assert {all c: Class | #c.inherits <= 1} is sat for 5 Class // but singleInheritance
}

test suite for multipleInheritance {
    // 10. Multiple inheritance: more than one parent possible
    assert {some c: Class | #c.inherits > 1} is sat for 5 Class // but multipleInheritance
}
